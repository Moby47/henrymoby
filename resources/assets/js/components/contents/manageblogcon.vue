...
<div v-if='load'>
 ...
 <input type="submit" @click.prevent='reverse()' v-if='reverseBtn' value="Reverse">  
          ...
          <v-img :src="'/storage/blog/'+con.image" class="img-thumbnail img-responsive size" alt=""
...
<button class='btn btn-info' @click="fetch(pagination.next_page_url)" :disabled="!pagination.next_page_url" :loading='loading'>
 Next
    ...
    axios.post('/save-blog',formdata).then(res=>{
  if(res.data == 1){
    
      this.$toasted.show("Blog Posted!");
      //clear in 3 secs
			setTimeout(func=>{
			this.$toasted.clear();
			},3000)
 ...
 fetch(page_url){
   if(page_url){
      ...
      NProgress.start()
          ...
          var   page_url = page_url || '/blog-content';
         ...
         NProgress.done()
          ...
          .catch(error =>{
         this.$toasted.show("Blog data failed...Please reload page");
            NProgress.done();        
              })
        },
...
//send to database with axios
axios.post('/update-blog',formdata).then(res=>{
            if(res.data == 1){
              
                this.$toasted.show("Blog Updated!");
                //clear in 3 secs
			setTimeout(func=>{
			this.$toasted.clear();
			},3000)
...
removeProj(id){
  ...
  axios.post('/destroy-blog',input)
  .then(res=>{
    if(res.data == 1){
        this.$toasted.show("Blog post deleted!");
